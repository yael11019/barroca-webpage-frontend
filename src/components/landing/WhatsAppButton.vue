...
const NUMERO = '524433396659'

// Mensaje predefinido + etiqueta de analytics según la vista desde la que se
// abre WhatsApp. Así el cliente llega con un texto contextual a su intención.
const MENSAJE_POR_RUTA: Record<string, { texto: string; contexto: string }> = {
  inicio: {
  ...
  agendar: {
    texto: 'Hola, me gustaría agendar una cita con un vendedor.',
    contexto: 'Agendar cita',
  },
  proyectos: {
    texto: 'Hola, me gustaría platicar sobre un proyecto.',
    contexto: 'Proyectos',
  },
  'carrito-vista': {
    texto: 'Hola, me gustaría finalizar mi pedido.',
    contexto: 'Carrito',
  },
  'barroca-bot': {
...
const mensaje = computed(() => {
  const nombre = route.name as string | undefined
  return (nombre && MENSAJE_POR_RUTA[nombre]) || MENSAJE_DEFAULT
})

const href = computed(
  () => `https://wa.me/${NUMERO}?text=${encodeURIComponent(mensaje.value.texto)}`,
)
</script>
...
<template>
  <a
    ...
    @click="trackWhatsAppClick(mensaje.contexto)"
  >
